...
def instantiate_llm_for_eval(settings: Settings) -> LLM:
    ...
    llm_eval_azure_open_ai = AzureOpenAIBasedLLM(
        ...
        model_name=settings.AZURE_OPEN_AI_LLM_EVAL_MODEL_NAME,
    ...
    async def run(self) -> None:
        ...
        for output in outputs:
            for chunk in output.retrieved_chunks:
                total_character_count += len(chunk.text)
        avg_character_count = total_character_count / len(outputs)

        logger.debug(
            f"Total character count of retrieved chunks: {total_character_count}"
        )
        logger.debug(
            f"Average character count of retrieved chunks: {avg_character_count}"
        )
        ...
        total_compute_times_ms = sum(compute_times_ms)
        total_compute_time_sec = int(total_compute_times_ms / 1000)
        avg_compute_time_sec = total_compute_time_sec / len(outputs)

        logger.debug(
            f"Total compute time for pipeline is {total_compute_time_sec} seconds."
        )
        logger.debug(
            f"Average compute time for pipeline is {avg_compute_time_sec} seconds."
        )
        ...
        search_quality_weight = 0.55
        total_character_count_weight = 0.15
        response_time_weight = 0.2
        answer_quality_weight = 0.1

        # Normalize metrics
        normalized_llm_score = mean_llm_score / 5
        normalized_mrr = mean_reciprocal_rank  # No need to normalize reciprocal rank

        ideal_response_time = 0.5  # seconds
        if avg_compute_time_sec <= ideal_response_time:
            normalized_response_time = (
                1 + (ideal_response_time - avg_compute_time_sec) / ideal_response_time
            )
        else:
            normalized_response_time = math.exp(
                -(avg_compute_time_sec - ideal_response_time) / ideal_response_time
            )
        normalized_response_time = max(0, min(normalized_response_time, 1))

        ideal_character_count = 3000
        if avg_character_count <= ideal_character_count:
            # Linear reward for shorter times
            character_count_score = (
                1
                + (ideal_character_count - avg_character_count) / ideal_character_count
            )
        else:
            # Exponential penalty for longer times
            character_count_score = math.exp(
                -(avg_character_count - ideal_character_count) / ideal_character_count
            )
        # Ensure the score is between 0 and 1
        normalized_character_count_score = max(0, min(character_count_score, 1))

        # Calculate weighted score
        weighted_score = (
            search_quality_weight * normalized_mrr
            + total_character_count_weight * normalized_character_count_score
            + response_time_weight * normalized_response_time
            + answer_quality_weight * normalized_llm_score
        )

        # You can add more detailed logging here
        logger.debug(f"Normalized MRR (Search Quality): {normalized_mrr:.4f}")
        logger.debug(f"Normalized Response Time: {normalized_response_time:.4f}")
        logger.debug(
            f"Normalized LLM Score (Answer Quality): {normalized_llm_score:.4f}"
        )
        logger.debug(
            f"Normalized Character Count Score (Total Character Count): {normalized_character_count_score:.4f}"
        )
        logger.debug(f"Weighted Score: {weighted_score:.4f}")
        #JSTE Custom <
        global Score_of_run
        Score_of_run = weighted_score
        # >JSTE Custom End
...
# Usage example:
Path_Man = "data/pipelines/naive-420-overlap-200-retriever-vector-generator-gpt4o-v1"
async def main():
    eval = PipelineEvaluator(
        pipeline_results_path=Path(
            Path_Man + "/result.jsonl"
        ),
...
if __name__ == "__main__":
    anyio.run(main)
    with open(Path_Man + "/Judge_Last.txt", 'w') as score:
        score.write(str(Score_of_run))
